...
#maybe I can have these line commands manipulate a 2d array, increasing the array based on commands.
#look for intersections by looking at where the array is greater than 2
import numpy as np
import sys
np.set_printoptions(threshold=sys.maxsize,linewidth=160)
...
for i in range(len(line1)):
    #print("xy",x,y)
    if line1[i][0]=='R':
        field[y][x+1:x+int(line1[i][1:])+1]=1
        x=x+int(line1[i][1:])
    elif line1[i][0]=='L':
        field[y][x-(int(line1[i][1:])):x]=1
        x=x-(int(line1[i][1:]))
    elif line1[i][0]=='U':
        field[y-(int(line1[i][1:])):y,x]=1
        y=y-(int(line1[i][1:]))

    elif line1[i][0]=='D':
        field[y+1:y+int(line1[i][1:])+1,x]=1
        y=y+int(line1[i][1:])
    if x<minx:
        minx=x
    if y<miny:
...
for i in range(len(line2)):
    #print("xy",x,y)
    if line2[i][0]=='R':
        field2[y][x+1:x+int(line2[i][1:])+1]=1
        x=x+int(line2[i][1:])
    elif line2[i][0]=='L':
        field2[y][x-(int(line2[i][1:])):x]=1
        x=x-(int(line2[i][1:]))
    elif line2[i][0]=='U':
        field2[y-(int(line2[i][1:])):y,x]=1
        y=y-(int(line2[i][1:]))
    elif line2[i][0]=='D':
        field2[y+1:y+int(line2[i][1:])+1,x]=1
        y=y+int(line2[i][1:])

#print(field)
#take max differences between origin and nearest index with value 2
min=10000
for i in range(miny,maxy):
    for j in range(minx,maxx):
        if field[i][j]==1 and field2[i][j]==1:
            if abs(origin[0]-i)+abs(origin[1]-j)<min:
                min=abs(origin[0]-i)+abs(origin[1]-j)
                print(i,j)
print(min)
